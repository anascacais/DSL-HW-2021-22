import numpy as np
from hw2_decoder import viterbi, forward_backward, mrd
...
def main():
    ...
    viterbi_weather = [weather[int(i)] for i in viterbi_path]
    ...
    mrd_path = mrd(
        initial_scores, transition_scores, final_scores, emission_scores)
    mrd_weather = [weather[int(i)] for i in mrd_path]

    print("Sequence given by Viterbi: %s" % " -> ".join(viterbi_weather))
    print("Sequence given by minimum risk decoding: %s" % " -> ".join(mrd_weather))
